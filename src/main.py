...
class StockResearchSystem:
    ...
    def __init__(self):
        ...
        # Initialize database
        self.db = initialize_database(config.database.database_url)
    ...
    async def fact_check_stock(self, symbol: str, check_type: str = "assess_data_quality") -> dict:
        """Fact-check data quality for a specific stock"""
        self.logger.info(f"Starting {check_type} fact-check for stock: {symbol}")
        
        if "fact_checker" not in self.agents:
            raise RuntimeError("Fact checker agent not available")
        ...
        # Submit task and wait for completion
        await fact_checker_agent.add_task(task)
        
        # Wait for processing
        await asyncio.sleep(3)
        
        self.logger.info(f"Fact-check completed for {symbol}")
        return {"status": "completed", "symbol": symbol, "check_type": check_type}
